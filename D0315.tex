...
% CWG FEEDBACK FROM KONA:
% - Rebase on top of C++17 DIS, and pay special attention to re-numbering sections & al.
% - In section 4.2, I should perhaps change the example to something like
%     template <unsigned N>
%     void f(const char (*s)[([]() { return 1; })() + N]) { }
%   because otherwise the body of the lambda is not part of the full expression inside the decltype. The example in section 6.3 needs to be fixed too.
% - In section 6.3, need to extend the wording to "Two lambda expressions involving within their bodies template parameters"
% - Clarify that we want something like
%     template <int N>
%     void foo2(int (*)[N], decltype([]{}));
%   to work. The idea is that we mangle that positionally based on the enclosing declaration.
...
\end{codeblock}
  ...
  \item \label{wording.template} \hyperref[discussion.template]{(discussion)}
    ...
    \begin{quote}
      ...
      parameter in the other expression. \added{Unless both expressions occur such
      that they are corresponding counterparts of each other within definitions of
      the same enclosing entity in separate translation units (\textbf{[basic.def.odr]}),
      two lambda-expressions appearing in full expressions involving template
      parameters are never considered equivalent. [ \textit{Note:} The intent is
      to avoid lambda-expressions appearing in the signature of a function template
      with external linkage. \textit{-- end note} ]}
    \end{quote}

    Also, add the following example after \textbf{[temp.over.link] 14.5.6.1/5}:
\begin{codeblock}
@\added{
// The program is ill-formed, no diagnostic required, because the two\\
// lambda-expressions are functionally equivalent but not equivalent.\\
// translation unit 1:\\
template <unsigned N>\\
