...
If an extraction function \href{http://eel.is/c++draft/input.streams\#istream.general-3}{([istream.general])}
returns \texttt{traits::eof()}, then \texttt{ios_base::eofbit} is set in
...
set in \added{the local error state, \texttt{*this}'s error state is set
...
the local error state, and may throw an exception.} In any case, the
...
array. \added{If an extraction function \href{http://eel.is/c++draft/input.streams\#istream.general-3}{([istream.general])}
returns \texttt{traits::eof()}, then \texttt{ios_base::eofbit} is set
in the local error state and the input function stops trying to obtain
the requested input.} If an exception is thrown during input then \texttt{ios_base::badbit}
is turned on in \changed{\texttt{*this}'s error state}{the local error
state, \texttt{*this}'s error state is set to the local error state, and
the exception is rethrown if \texttt{(exceptions() \& badbit) != 0}}.
\removed{(Exceptions thrown from \texttt{basic_ios<>::clear()} are not
