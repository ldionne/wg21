...
The proposed wording changes are relative to the C++26 working draft [N5001](https://wg21.link/N5001) with [P2900R14](https://wg21.link/P2900R14) already applied.

## Introduce hardened preconditions

Add a new paragraph to [intro.compliance]{.sref} after paragraph 7 as indicated:

> [7]{.pnum} [Two kinds of implementations are defined:]{.rm}[An implementation is either]{.add} a _hosted implementation_ [and]{.rm}[or]{.add} a _freestanding implementation._ A freestanding implementation is one in which execution may take place without the benefit of an operating system. A hosted implementation supports all the facilities described in this document, while a freestanding implementation supports the entire C++ language described in `[lex]` through `[cpp]` and the subset of the library facilities described in `[compliance]`.

:::add
> [8]{.pnum} It is implementation-defined whether the implementation is a _hardened implementation_. If it is a hardened implementation, violating a hardened precondition results in a contract violation `([structure.specifications])`.
:::

Add a new element to [structure.specifications]{.sref} after element 3.3 as indicated:

> [3.3]{.pnum} _Preconditions_: [the]{.rm} conditions that the function assumes to hold whenever it is called; violation of any preconditions results in undefined behavior.

:::add
> [3.4]{.pnum} _Hardened preconditions_: conditions that the function assumes to hold whenever it is called.
>
...
Add to [structure.specifications]{.sref} in paragraph 4:

> Whenever the _Effects_ element specifies that the semantics of some function `F` are _Equivalent to_ some code sequence, then the various elements are interpreted as follows. [...] Next, the semantics of the code sequence are determined by the _Constraints_, _Mandates_, _Preconditions_, [_Hardened preconditions_,]{.add} _Effects_, _Synchronization_, _Postconditions_, _Returns_, _Throws_, _Complexity_, _Remarks_, and _Error conditions_ specified for the function invocations contained in the code sequence. [...]
...
::: add
> [?]{.pnum} Additionally, each of the following macros is defined in a hardened implementation:
:::
