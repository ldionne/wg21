...
> [7]{.pnum} [Two kinds of implementations are defined:]{.rm}[An implementation may be]{.add} a _hosted implementation_ [and]{.rm}[or]{.add} a _freestanding implementation._ A freestanding implementation is one in which execution may take place without the benefit of an operating system. A hosted implementation supports all the facilities described in this document, while a freestanding implementation supports the entire C++ language described in `[lex]` through `[cpp]` and the subset of the library facilities described in `[compliance]`.

:::add
> [8]{.pnum} Additionally, an implementation can be a _hardened implementation_. A hardened implementation is one in which violating a _hardened precondition_ results in a contract violation `([structure.specifications])`.
:::
