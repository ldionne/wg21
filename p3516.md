...
```c++
template <class T>
void relocate-at(T* source, T* dest); // exposition-only
```

_Preconditions_: `source` and `dest` point to different objects
...
_Preconditions_: `result` is not in the range `[first, last)`
...
constexpr NoThrowForwardIterator
uninitialized_relocate_n(InputIterator first,
                         Size n,
                         NoThrowForwardIterator result);
```

_Preconditions_: `result` is not in the range `[first, first + n)`
...
TODO: Finish wording for ranges and parallel algorithms
...
[`[specialized.algorithms]`]: http://eel.is/c++draft/specialized.algorithms
