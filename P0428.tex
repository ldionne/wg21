...
\begin{quote}
  ...
  If the \textbf{auto} \textit{type-specifier} appears as one of the
  \textit{decl-specifiers} in the \textit{decl-specifier-seq} of a
  \textit{parameter-declaration} of a \textit{lambda-expression}, or if
  the lambda has a \textit{template-parameter-list}, the lambda is a
  \textit{generic lambda}.\\
  [ \textit{Example:}\\
  \tcode{auto glambda1 = [](int i, auto a) \{ return i; \}; // OK: a generic lambda}\\
  \tcode{auto glambda2 = []<class T>(T t, int i) \{ return i; \}; // OK: a generic lambda}
  -- \textit{end example} ]
  }
\end{quote}
