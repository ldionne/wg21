...
<!-- Feedback to apply:
- motivation section should explain that we can specialize operator new for a whole family of types

- This is a better design instead:
    template <class T>
    void* operator new(std::typed_alloc_t<T>, size_t)

    void* operator new(std::typed_alloc_t<MyFoo>, size_t)

    Also we probably don't need to sandwich with align_val_t since we know the alignment of T now.
-->
