...
- R2:
  - Added the initial spec wording
- R3:
  ...
  - Added new design choice sections for feedback from EWG
    - [Design choice](#design-choice-safety-restriction-on-mismatched-declaration-scopes) for
      cleanup deallocation functions needing to be in the same scope as the allocation
    - [Design choice](#design-choice-permit-sized-delete-when-performing-exception-cleanup-delete)
      for leveraging this new syntax to support sized placement delete as requested in [@P3492R0]
  - Explicitly mention that qualifiers are discarded on the type being passed
  - More comprehensive wording
    ...
    - Update [expr.const] include language to permit use of type aware functions in constant
      expressions
...
> represents the type being allocated, such that when allocating an object or array of type `T`, the
...
> [21]{.pnum old=20} Overload resolution is performed on a function call by assembling an argument list.

[these points exist, but are being inset and numbered/put into a sub-list]{.draftnote}

>  - [21.1]{.pnum old=""} [The first argument is the type-identity argument, the second]{.add}[The
>    first]{.rm} argument is the amount of space requested, and has type `std::size_t`.
>  - [21.2]{.pnum old=""} If the type of the allocated object has new-extended alignment, the next
...
>
>    - [21.2.3]{.pnum old=""} and then overload resolution is performed again.
...
> [30]{.pnum} If a new-expression calls a deallocation function it passes [the type-identity argument as the
...
> replaceable global allocation function]{.add} and the allocated storage is deallocated within
> the evaluation of E, or
...
context requires that the resolved operator be `::operator new` / `::operator delete`. In particular,
if an in-class `T::operator new` is defined and the _new-expression_ resolves to it, the
_new-expression_ is not `constexpr`-friendly. That seems to be an arbitrary limitation.
...
the lack of any mechanism to otherwise force co-definition of new and delete operators. In [expr.new
]{.unnumbered .sref} we would like to add the follow paragraph to force the selected deletion
operator
...
## Design choice: Permit sized delete when performing exception cleanup delete

[@P3492R0] Discusses the problem of placement delete being unable to call sized delete. As a new
syntax and function signature there is no risk of conflict with existing code. We could resolve
[@P3492R0] by permitting a sized delete to be called during cleanup of placement new. The wording
currently lacks this, and we lack direct implementation experience, but introducing support today
might reduce problems in the future.

The wording changes would likely be to [expr.new]{.sref .unnumbered}, something along the lines of

> [29]{.pnum} A declaration of a placement deallocation function matches the declaration of a
> placement allocation function if it has the same number of parameters and, after parameter
> transformations ([dcl.fct]{.unnumbered .sref}), all parameter types except [the first]{.rm}[the
> size and object]{.add}[, and optionally a parameter of type `std::size_t` immediately
> following the address parameter]{.add} are identical.

An alternative would be to mandate all currently "optional" parameters become mandatory for type
aware allocation functions, however we feel that that considerably increases mental load of the
feature, and more over that mandates sized delete, even though there are cases where it is not
possible for the author or compiler to know the size of the object ahead of time.
...
resolve type aware new or delete. The coroutine specification will need to be updated in the future to
...
This proposal has exposed a number of gaps in the current language specification, that would ideally
be addressed in future proposals.

## Coroutine frame types

Coroutines explicitly resolve `operator new` and `operator delete` independently of other resolution
paths, but even when that resolution is performed they do not provide any type information about the
allocation being performed. From discussions with multiple vendors, the exact content of the promise
frame being allocated is not determined until _after_ front end and semantic analysis has occured: the
content of the type being allocated is determined by the codegenerator.

Given those semantics it seems implausible that we will ever be able to provide a type that
accurately reflects the exact type being allocated, however for type aware allocators being able to
distinguish a coroutine frame from an opaque bucket of bits is critical, as coroutine frames are
analagous to stack frames and can contain critical data - code pointers, post validation spills,
etc - as well as being subject to all the historical attacks applied to any other stack frame.

At minimum some degree of tag type will be needed, but it seems likely that at least some metadata
and queries should be possible prior to codegen, so the exact design of the type requires discussion
that is outside the scope of this proposal.

## constexpr allocation functions

In order to support functional requirements of a system allocator, this proposal adds wording to
special case globally defined type aware allocation functions that are filling the space usually
reserved for the _global replaceable allocation functions_. This wording is necessary as the current
specification language explicitly precludes any _new-_ or _delete-expression_ where the resolved
operator is not explicitly a globally replaceable allocation function.

This restriction is not necessary as constexpr `new` and `delete` are both implementable via the
constexpr `std::allocator` API, or via direct calls to `::operator new(...)` and
`::operator delete(...)`. As a result of this restriction no class, or class hierarchy, may be used
during constant expression evaluation, because the mere existence of the class scoped allocation
function precludes lookup from ever reaching the global scope. This means that any scenarios where
such a type must be used in a constant evaluation context must be forced to use a global operators,
potentially leading to security or correctness errors, or must use copious `if consteval` blocks
through out otherwise clean code.

# Acknowledgments

We'd like to acknowledge the time spent by Jens Meurer, Brian Bi, Corentin Jabot, Richard Smith,
