...
indenting and adding paragraph numbers to steps in paragraph 20. The below removal is the section of text that is being inset and separated into steps below.
:::
...
or `operator delete`. Instead, using a _new-expression_ or a _delete-expression_ in a `constexpr` context requires that
the resolved operator be `::operator new` / `::operator delete`. In particular, if an in-class `T::operator new` is defined and the _new-expression_ resolves to it, the _new-expression_ is not `constexpr`-friendly. That seems to be an arbitrary limitation.
...
We clearly don't want or need such a limitation for typed `operator new`. Our solution to that is to treat a global typed `operator new` (without placement parameters) just like an untyped `::operator new`, i.e. as a _replaceable global allocation function_. While that does mean the compiler will assume the behaviour of typed `operator new`, that seems like a reasonable assumption inside `constexpr`.
...
- allow constexpr type aware allocation functions in constant evaluation, as the functions have the
  ...
  ```cpp
  constexpr void *operator new(std::type_identity<X> type, size_t sz) {
    if consteval {
