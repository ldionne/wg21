...
is both supported by all known implementations and also better understood by users than e.g. weak definitions.
...
explaining a technique used inside its kernel (XNU) to mitigate various exploits. At its core, the technique consists in
...
namespaces of `T` (by virtue of the template parameter of `std::type_identity`), and that of any other provided arguments. In
particular, note that this may or may not include the global namespace.
...
Delete expressions work in a way entirely analoguous to what is described above for new-expressions.
...
## Note: Is the current name lookup for `T::operator new` done this way on purpose?
...
`T::operator new` but forgets to define some other variants. [For example](https://godbolt.org/z/ahW6Eq96r):
