...
<!-- TODO: Remove "note 2" and remove wall of text -->

Paragraph 2, Note 2:

> The library provides default definitions for the global allocation and deallocation functions.
> Some  global allocation and deallocation functions are replaceable ([new.delete]); these are
> attached to the global module ([module.unit]). A C++ program shall provide at most one definition
> of a replaceable allocation or deallocation function. Any such function definition replaces the
> default version provided in the library ([replacement.functions]). The following allocation and
> deallocation functions ([support.dynamic]) are implicitly declared in global scope in each
...
> [std::type_identity, ]{.add}, or any other names that the library uses to declare these names.
> Thus, a new-expression, delete-expression, or function call that refers to one of these functions
> without importing or including the header <new> or importing a C++ library module ([std.modules]) is
> well-formed. However, referring to std or std​::​size_t or std​::​align_val_t is ill-formed unless a
> standard library declaration ([cstddef.syn], [new.syn], [std.modules]) of that name precedes
> ([basic.lookup.general]) the use of that name. — end note]{.note}

::: add
> [3]{.pnum} Allocation and/or deallocation functions may be _type-aware_. A type-aware allocation or
...
> [3->4]{.pnum} If the behavior of an allocation or deallocation function does not satisfy the semantic
> constraints specified in [basic.stc.dynamic.allocation] and [basic.stc.dynamic.deallocation], the
...
> type std​::​size_t ([support.types]{.sref}). The [first]{.rm}[_size_]{.add} parameter shall not have
> an  associated default argument ([dcl.fct.default]{.sref}). The value of the
> [first]{.rm}[_size_]{.add} parameter is interpreted as the requested size of the allocation.
> An allocation function can be a function template. Such a template shall declare its return type
> and [first]{.rm}[_size_]{.add} parameter as specified above (that is, template parameter types shall
> not be used in the return type and [first]{.rm}[_size_]{.add} parameter type). Allocation function
...
::: add
> [3]{.pnum} A deallocation function is a type-aware deallocation function if the first parameter is of
> type `std::type_identity<T>` ([support.types]{.sref}) for some type `T`. This parameter shall not have
> an associated default argument. A type-aware deallocation function shall not be a _destroying operator
> delete_.
:::

> [3->4]{.pnum}Each deallocation function shall return void. [The _subject_ parameter of a type-aware
...
> [shall only be a usual deallocation function if it is a type-aware deallocation function, and the only
...
> If the type of the allocated object has new-extended alignment, the next argument is the type's
> alignment, and has type std​::​align_val_t. If the new-placement syntax is used, the
> initializer-clauses in its expression-list are the succeeding arguments. If no matching function is
> found then
>
...
>    is added into the argument list immediately after the first argument; and then overload resolution is
>    performed again.
...
> transformations ([dcl.fct]), all parameter types except [the first]{.rm}[if the function is type
> aware the second; otherwise the first] are identical.

> [30]{.pnum} If a new-expression calls a deallocation function [it passes the value returned from
> the allocation function call as the first argument of type void*.]{.rm}

::: add
>  - if the allocation function is type-aware it passes a default initialized instance of
>    `std::type_identity<T>` where `T` is the type being allocated as the first parameters,
>    and the value returned from the allocation function as the second parameter; otherwise
...
::: add
>  - [9.2]{.pnum} If any of the deallocation functions are type-aware, and may be called with a first parameter
>    of type `std::type_identity<T>` where `T` is the static type of the object being deleted, all deallocation
>    functions that are not type-aware operator deletes are eliminated from further consideration.
:::
...
> [10]{.pnum} .... [If a type-aware deallocation function is used, a default initialized instance of
> type `std::type_identity<T>`, where `T` is the static type of the object being deleted, is passed as
> the first parameter.]{.add}
