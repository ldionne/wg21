...
> first parameter is of type `std::type_identity<U>` for some type `U` there shall be at least three
> parameters; the first parameter is called the _type-identity parameter_, the second parameter is
> called the _size parameter_, and the third parameter is called the _alignment parameter_. Otherwise,
> the first parameter is called the _size parameter_ and shall have type
> `std::size_t`([support.types]{.unnumbered .sref}).]{.add}
...
> `cvr T`, the type-identity parameter shall be of type `std::type_identity<T>`.]{.add} The value of 
...
> have [more than one parameter]{.rm}[additional parameters beyond the type-identity, address, size,
> and alignment parameters]{.add}.
...
> dependent type), the type-identity parameter is the only dependent parameter, and there are no
> template packs in the parameter list.]{.add}
