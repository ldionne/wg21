...
`template <template<typename> typename> void *operator new(T<int>, std::size_t)`.]{.draftnote
audience=CWG}
...
[This text is a significant change in specification, but not intended behavior vs what was presented
in Wroclaw.]{.draftnote audience=EWG}

[Brian pointed out that it's plausible in the future we might add constexpr new/delete to support
custom allocators in general, and was wondering how these terms would apply in such a case. We
believe that such a change would be easily accomodated by only apply this rule to declarations that
lacked the constexpr annotation (we discussed more general solutions in the [design
choices](#design-choice-normal-global-type-aware-allocation-function-are-allowed-in-consteval-contexts)).
...
## Design choice: "normal" global type aware allocation function are allowed in consteval contexts

In the current specification it is not possible for an author to control whether an
operator new or delete is selectable in consteval contexts, yet the use of `new` and
`delete` in such contexts requires that the resolved operator be a replaceable global
 allocation function [expr.const]{.unnumbered .sref}.

 This already causes problems for class scoped operators as the existence of a
 correctly named operator in the class scope prevents any further lookup in the global
 scope. However it is possible for code that wishes to support consteval and custom
 operators with code of the form:
...
- allow dynamic allocation in constexpr contexts via calls to `::operator new(size_t)` and
  `::operator delete(void*)`, and then permit use of constexpr operator `new` or `delete`.
  This would allow custom allocators _if_ they are inline, even when not type aware. e.g
  ```cpp
  class Foo {
...
[This is a significant semantic change since Wroclaw following feedback from reviewers, however
it has no change in implementation complexity and - from other feedback - seems to be what people
had already expected]{.draftnote audience=EWG}

There is a question as to whether the type-identity should include qualifiers. In preceding drafts
this was not addressed, and retaining the qualifiers was implied. Our belief at the time was that
an author that wished to strip the qualifiers would always be able to. Since then it has been
pointed out that this may cause confusion as qualifiers currently do not impact allocator
selection, and as the common case for allocation does not involve qualifiers on the type being
allocated may lead to foot-guns where an author believes they have defined a type aware allocator,
but that allocator is then not used in some cases.

While considering this we realised an additional hazard, consider:

```cpp
  struct Example;
  void *operator new(std::type_identity<Example>, size_t);
  void operator delete(std::type_identity<Example>, void *);
  struct Example {
    virtual ~Example();
  }
  void f() {
    auto e = new const Example;
    delete e;
  }
```

In this case the type aware `operator new` will not be called due to the qualifier mismatch, but the
type aware `operator delete` will be called from the virtual destructor. This is clearly not the
intended behavior.

This latter case indicates a degree of fragility that comes from not removing qualifiers that we
have realised that removal of qualifiers is the correct course of action.
