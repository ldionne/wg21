...
- R5 (CWG in Hagenberg):
  - Updated wording to reflect EWG guidance on Monday:
...
::: add
> [1]{.pnum} An allocation function or function template shall have at least one parameter,
> and shall have a return type specified as the non-dependent type `void *`. An allocation
> function or function template that is not a class member shall belong to the global scope. An
...
>
> [1.1]{.pnum} If the first parameter of an allocation function or function template is of type
> `std::type_identity<U>` ([meta.trans.other]{.unnumbered .sref}) for some possibly dependent
> type `U` it is termed _type aware_. A type aware allocation function or function template shall
> have at least three parameters. The first parameter is termed the _type-identity_. The second
> parameter shall be of the non-dependent type `std::size_t` ([support.types]{.unnumbered .sref})
> and is termed the _size parameter_. The third parameter is of the non-dependent type `std::align_val_t` 
> ([support.types]{.unnumbered .sref}) and is termed the _alignment parameter_. None of these first
> three parameters may be specified with default arguments ([dcl.fct.default]{.unnumbered .sref}).
> If a type aware allocation function is declared in a non-global scope, a type aware deallocation
> function with an equivalent array or non-array name accepting a type-identity parameter of the
> same type shall also be declared in the same scope. If the type-identity parameter is a dependent
> type it is ill-formed with no diagnostic required if there is no matching type aware deallocation
> function.
:::

:::draftnote

This language, coupled with complimentary language in [basic.stc.dynamic.deallocation]{.unnumbered .sref} is intended
...
::: add
> [1.2]{.pnum} If the first parameter of an allocation function or allocation function template
> is not of type `std::type_identity<U>` ([meta.trans.other]{.unnumbered .sref}) for some type `U`, the
> allocation function is _type oblivious_, and has no specified type-identity parameter. The first
> parameter shall have the non-dependent type `std​::​size_t` ([support.types]{.unnumbered .sref}) and is
> termed the _size parameter_. The first parameter shall not have an associated default argument ([dcl.fct.default]{.unnumbered .sref}).
> If the second parameter of an allocation function is of type `std::align_val_t`
> ([support.types]{.unnumbered .sref}) this parameter is termed the _alignment parameter_.
>
> [1.3]{.pnum} For an allocation function or function template the size parameter is interpreted
> as the requested size of the allocation. In type aware allocation functions and function templates
> the static `type` member of the type-identity parameter type is interpreted as the type the
> allocation is being requested for.
:::
...
>
>  - [3.1]{.pnum} If the allocation function or allocation [takes an argument of type std​::​align_val_t]{.rm} [has an
...
> function shall have at least one parameter. If the first parameter is of type `std::type_identity<U>`
> ([meta.trans.other]{.unnumbered .sref}) the deallocation function is _type aware_, and there shall
> be at least four parameters; the first parameter is termed the _type-identity parameter_,
> the second parameter is termed the _address parameter_, the third parameter has the non-dependent
> type `std::size_t`, and the fourth has the non-dependent type `std::align_val_t`. Otherwise, the
> deallocation function is _type oblivious_ and the first parameter is termed the 
> _address parameter_.]{.add}. If the function is a destroying operator delete declared in class
> type `C`, the type of  its [first]{.rm}[address]{.add} parameter shall be `C*`; otherwise, the
...
> only a usual deallocation function if it is type aware, the type-identity parameter is a dependent
> type (i.e. the type-identity parameter is of type `std::type_identity<U>` where `U` shall be a
> dependent type), and the type-identity parameter is the only dependent parameter.]{.add}
...
Is this sufficient to prevent a case like this example from being a usual deallocation function? It seems
like the wording might permit the below if Args is an empty pack?

```cpp
template <typename U, typename... Args>
void operator delete(type_identity<U>, void*, std::size_t, std::align_val_t, Args...);
```
:::
...
>   ]
>
> [30]{.pnum old="29"} [A declaration of a type aware placement deallocation function matches the
> declaration of a type aware placement allocation function if after parameter transformations
