...
::: draftnote
In principle paragraph 32 should not be necessary, as the language in
...
to mismatching operators new and delete, p32 makes it explicit that it is never permissible for the
cleanup delete to be resolved from a different scope if the original allocation function is type aware.
:::
...
::: draftnote
 p10 below is trying to update the existing overload resolution semantics to say
  ...
  - Ensure that the type-identity is the correct type, e.g even if there was a hypothetical
    conversion path from `std::type_identity<U>` to `std::type_identity<V>` that would not
    be permitted.
...
>    is the same type as the type-identity parameter, the deduced template-arguments are used to
...
The following implies an attempt to resolve a type aware deallocation function without size or
alignment parameters. It does this to reduce complexity and change to the specification language,
however as the declaration of such an operator is invalid, no such resolution can ever occur.
