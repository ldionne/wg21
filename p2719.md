...
> `std::type_identity<U>` for some possibly dependent type `U` it is termed _type aware_. A type
> aware allocation function or function template shall have at least three parameters. The
> first parameter is termed the _type-identity_. The second parameter shall be of the 
> non-dependent type `std::size_t` ([support.types]{.unnumbered .sref}) and is termed the
> _size parameter_. The third parameter is of the non-dependent type `std::align_val_t` 
> ([support.types]{.unnumbered .sref}) and is termed the _alignment parameter_.
> None of these first three parameters may be specified with default arguments.
> ([dcl.fct.default]{.unnumbered .sref}). If a type aware allocation function is declared in a
> non-global scope, a type aware deallocation function with an equivalent array or non-array
> name accepting a type-identity parameter of the same type shall also be declared in the same
...
> is not of type `std::type_identity<U>` for some type `U`, the allocation function is _type oblivious_,
> and has no specified type-identity parameter. The first parameter shall have the non-dependent type
> `std​::​size_t` ([support.types]{.unnumbered .sref}) and is termed the _size parameter_. The first
> parameter shall not have an associated default argument ([dcl.fct.default]{.unnumbered .sref}).
