...
\begin{enumerate}
  ...
  Fortunately, this specific problem can't arise, even with the above removal
  of constraints on lambda-expressions. Indeed, according to \textbf{[basic.link]
  3.5/8}, closure types have no linkage, and therefore (by the second part of
  that paragraph) they cannot be used to declare functions with external linkage.
  To make it clear that closure types are never given a name for linkage purposes,
  we propose modifying \textbf{[decl.typedef] 7.1.3/9} as follows:
  \begin{quote}
...
\end{lstlisting}
  ...
  \item Another possible concern that quickly follows is the appearance of
  lambda-expressions in contexts that are constrained by the ODR. For example:
...
\end{cpp}
  ...
  \item In template function declarations:
    [TODO]
